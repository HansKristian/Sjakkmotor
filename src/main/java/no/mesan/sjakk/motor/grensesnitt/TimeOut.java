...
public class TimeOut {
	 ...
	 *            interrupt()
	 */
	public TimeOut(final Runnable target, final long timeout,
		...
		barrier = new CyclicBarrier(2); // There will always be just 2 threads
	...
	public boolean execute() throws InterruptedException {
		...
		try {
			barrier.await(); // Need to wait on this barrier to make sure status
								// is set
	...
	private class Interrupter implements Runnable {
		...
		public void run() {
			try {
				Thread.sleep(timeout); // Wait for timeout period and then kill
										// this target
				if (forceStop) {
					target.stop(); // Need to use stop instead of interrupt
									// since we're trying to kill this thread
				} else {
					target.interrupt(); // Gracefully interrupt the waiting
										// thread
				}
			...
			try {
				...
				// If the Child and Interrupter finish at the exact same
				// millisecond we'll get here
